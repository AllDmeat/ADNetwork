...
`swift package clean && swift package resolve` resets build artifacts and re-fetches dependencies after editing `Package.swift`. Always run commands from the repo root so SwiftPM can locate `Package.swift`. GitHub Actions (`.github/workflows/ci.yml`) runs the Release build plus parallel tests on macOS 14, so duplicate that locally before pushing.
