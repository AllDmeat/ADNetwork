...
`swift package clean && swift package resolve` resets build artifacts and re-fetches dependencies after editing `Package.swift`. Always run commands from the repo root so SwiftPM can locate `Package.swift`. GitHub Actions (`.github/workflows/test.yml`) runs `swift build` + `swift test --parallel` on macOS 26 for every pull request, so duplicate that locally before pushing.
