...
`swift package clean && swift package resolve` resets build artifacts and re-fetches dependencies after editing `Package.swift`. Always run commands from the repo root so SwiftPM can locate `Package.swift`. GitHub Actions (`.github/workflows/test.yml`) runs `swift build` + `swift test --parallel` for every pull request, so duplicate those commands locally before pushing.
